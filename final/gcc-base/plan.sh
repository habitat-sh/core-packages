...
do_prepare() {
	...
	case $pkg_target in
	aarch64-linux)
		dynamic_linker="$(pkg_path_for glibc-base)/lib/ld-linux-aarch64.so.1"
		...
		;;
	x86_64-linux)
		dynamic_linker="$(pkg_path_for glibc-base)/lib/ld-linux-x86-64.so.2"
		HAB_GCC_STAGE1_GLIBC_DYNAMIC_LINKER="${dynamic_linker}"
		export HAB_GCC_STAGE1_GLIBC_DYNAMIC_LINKER
		build_line "Setting HAB_GCC_STAGE1_GLIBC_DYNAMIC_LINKER=${HAB_GCC_STAGE1_GLIBC_DYNAMIC_LINKER}"
		;;
	...
	# * -L$(pkg_path_for glibc-base)/lib
	#   Tells gcc where to find the C library, usually libc and libm.
	# * -idirafter $(pkg_path_for glibc-base)/include
	...
	FLAGS_FOR_TARGET="-B$(pkg_path_for glibc-base)/lib \
		-L$(pkg_path_for glibc-base)/lib \
		-idirafter $(pkg_path_for glibc-base)/include \
		-idirafter $(pkg_path_for linux-headers)/include"
...
do_build() {
	...
	../configure \
		...
		--with-native-system-header-dir="$(pkg_path_for glibc-base)/include" \
		--disable-bootstrap \
		--enable-default-pie \
		--enable-default-ssp \
		--disable-multilib \
...
wrap_binary() {
	...
	sed "$PLAN_CONTEXT/cc-wrapper.sh" \
		...
		-e "s^@glibc@^$(pkg_path_for glibc-base)^g" \
