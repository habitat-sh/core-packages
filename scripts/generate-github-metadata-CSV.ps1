...
$repoRootURL = "https://github.com/habitat-sh/core-packages"

# Create an array to store the results
$results = @()
# Directories to exclude
$excludeDirs = @('.github', 'docker', 'docs', "scripts", "workflows", "hab-bootstrap")

# Filter out excluded directories
$directories = $directories | Where-Object { $excludeDirs -notcontains $_.Name }

foreach ($dir in $directories) {
    Write-Debug "Processing directory: $($dir.FullName)"

    # Get relative directory path from the repository root
    $repoRoot = (Get-Item "..").FullName
    $relativeDirPath = $dir.FullName.Substring($repoRoot.Length).TrimStart('\', '/')
    ...
    if ([string]::IsNullOrWhiteSpace($planContent)) { 
        # Only warn if directory has no child directories
        $hasChildDirs = (Get-ChildItem -Path $dir.FullName -Directory -ErrorAction SilentlyContinue).Count -eq 0
        if ($hasChildDirs) {
            Write-Host "No plan content found in $($dir.FullName), skipping..."
        }
        continue 
    }

    # Write-Host "Plan content found in $($dir.FullName) at $(if ($planSh) { 'plan.sh' } else { 'plan.ps1' })"
    

    # generate a CSV row for a given directory/package
    $results += [PSCustomObject]@{
        # generate package, origin + package columns
        FolderName = $relativeDirPath
        ...
        GitHubURL = "$repoRootURL/tree/main/$relativeDirPath"
        LinuxPlanFileURL = if ($planSh) { "$repoRootURL/blob/main/$relativeDirPath/plan.sh" } else { "NO LINUX PLAN FILE" }
        WindowsPlanFileURL = if ($planPs1) { "$repoRootURL/blob/main/$relativeDirPath/plan.ps1" } else { "NO WINDOWS PLAN FILE" }
