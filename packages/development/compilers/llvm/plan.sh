...
pkg_source="https://github.com/llvm/llvm-project/releases/download/llvmorg-18.1.6/llvm-project-18.1.6.src.tar.xz"
pkg_shasum=bd4b4cb6374bcd5fc5a3ba60cb80425d29da34f316b8821abc12c0db225cf6b4
#pkg_dirname="${_distname}-${pkg_version}.src"
pkg_deps=(
  core/coreutils
  core/gcc-libs
  core/glibc
...
do_prepare() {
  ...
  _fix_interpreter_in_path "$pkg_prefix/src/llvm" '*.py' core/python bin/python
  _fix_interpreter_in_path "$pkg_prefix/src/llvm" '*.py' core/coreutils bin/env
  _fix_interpreter_in_path "$pkg_prefix/src/llvm" '*.sh' core/coreutils bin/env
}
...
do_build() {
  ...
  install compile_commands.json "${pkg_prefix}/src/llvm"
  popd || exit 1
}
