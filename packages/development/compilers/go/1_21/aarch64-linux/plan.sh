...
pkg_shasum=58f0c5ced45a0012bce2ff7a9df03e128abcc8818ebabe5027bb92bafe20e421
pkg_dirname=go
pkg_deps=(
  core/glibc
)
...
do_prepare() {
  ...
  sed -e "s,/lib/ld-linux-aarch64.so.1,${dynamic_linker}," \
    -i src/cmd/link/internal/arm64/obj.go
...
do_build() {
  pushd src > /dev/null || return 1
   file $(pkg_path_for $pkg_bootstrap_pkg)/bin/go
   $(pkg_path_for $pkg_bootstrap_pkg)/bin/go version
    bash -x make.bash --no-clean
