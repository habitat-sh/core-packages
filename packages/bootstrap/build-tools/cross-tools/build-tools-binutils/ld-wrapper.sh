...
HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH=${HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH:-${HAB_LD_RUN_PATH:-${LD_RUN_PATH:-""}}}
PREFIX=${PREFIX:-""}
...
if [[ $linkType != static-pie && -n $HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH ]]; then
	...
	for dir in ${libDirs+"${libDirs[@]}"}; do
		...
		for lib in "${!libs[@]}"; do
			...
			*)
			...
			if [[ ${libs[$lib]} == 2 && ":$HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH:" == *":$dir:"* ]]; then
				addDir=1
			fi
		done
	...
	if [[ -n $PREFIX && $missingLibs == 1 ]]; then
		IFS=: read -r -d '' -a ldPathArray < <(printf '%s:\0' "$HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH")
...
if (("${HAB_BUILD_TOOLS_BINUTILS_DEBUG}" >= 1)); then
	...
	echo "env HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH=${HAB_BUILD_TOOLS_BINUTILS_LD_RUN_PATH-}" >&2
