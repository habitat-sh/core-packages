...
#
# * `$HAB_DEBUG`, `$HAB_LD_DEBUG`, `$HAB_NATIVE_CROSS_BINUTILS_DEBUG` (*Optional*): If one of these is set,
...
HAB_NATIVE_CROSS_BINUTILS_DEBUG=${HAB_DEBUG:-${HAB_LD_DEBUG:-${HAB_NATIVE_CROSS_BINUTILS_DEBUG:-0}}}

if (("${HAB_NATIVE_CROSS_BINUTILS_DEBUG}" >= 7)); then
	set -x
fi
