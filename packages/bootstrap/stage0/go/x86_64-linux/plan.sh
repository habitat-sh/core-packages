...
do_prepare() {
  ...
  cat "$PLAN_CONTEXT/cacerts.patch" \
    | sed -e "s,@cacerts@,$(pkg_path_for build-tools-cacerts)/ssl/cert.pem,g" \
    | patch -p1
...
do_check() {
  ...
  local _clean_cmds=()
  if [[ ! -r /bin/pwd ]]; then
    ln -sv "$(pkg_path_for build-toolscoreutils)/bin/pwd" /bin/pwd
    _clean_cmds+=(/bin/pwd)
  fi
  ...
  pushd src > /dev/null
    env LD_LIBRARY_PATH="$(pkg_path_for gcc-stage0)/lib" bash run.bash --no-rebuild
  popd > /dev/null
